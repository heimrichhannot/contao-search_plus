var SearchPlus = {
     ...
     */
    toggleSearchablePages: function (el, id, field) {
        el.blur();
        var item = $(id);
        ...
        if (item) {
            if (!el.value) {
                ...
                new Request.Contao({field:el}).get({'action':'toggleSearchablePages', 'id':id, 'field':field, 'state':1, 'REQUEST_TOKEN':Contao.request_token});
            } else {
                el.value = '';
                el.checked = '';
                item.setStyle('display', 'none');
                new Request.Contao({field:el}).get({'action':'toggleSearchablePages', 'id':id, 'field':field, 'state':0, 'REQUEST_TOKEN':Contao.request_token});
            }
            return;
        }
        ...
        }).get({'action':'toggleSearchablePages', 'id':id, 'field':field, 'load':1, 'state':1, 'REQUEST_TOKEN':Contao.request_token});
    }
};